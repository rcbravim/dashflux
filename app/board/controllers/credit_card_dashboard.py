...
from app.database.models import Category, Establishment, Account, Transaction, Analytic, CreditCardTransaction
...
def credit_card_controller():
    ...
    if request.method == 'GET':
        ...
        entries_all = db.session.query(
            ...
            transaction.cct_amount,
            transaction.cct_entry_date,
            transaction.cct_description,
            transaction.category_ids
        ).join(
            establishment, transaction.establishment_id == establishment.id
        ).filter(
            transaction.cct_status == True,
            transaction.user_id == user_id,
            extract('month', transaction.cct_entry_date) == month,
            extract('year', transaction.cct_entry_date) == year
        ).order_by(
            transaction.cct_entry_date.asc()
        ).all()
        ...
        for row in entries_all:
            ...
            entry = {
                'id': row.id,
                'cct_entry_date': row.cct_entry_date,
                ...
                'cct_amount': row.cct_amount,
                'cct_description': row.cct_description,
        ...
        return render_template('board/pages/dashboard_credit_card.html', context=context, success=success)
